...
<?php foreach($schinfo->result() as $row)
	...
	if($ave<50)
	{
		echo "<p style='color: red;font-size: 12; border: 1px black solid; padding: 2px; font-weight:bold;'>FAILED</p>";
	}
	else if($ave>=50)
	{
		echo "<p style='color: green;font-size: 12; border: 1px black solid; padding: 2px; font-weight:bold;'>PASSED</p>";
	} ?></td> -->
	<td id="status"></td>
	</tr>
	</table>
	...
	<th style="color: #183292" colspan="3">Third Term Summary</th>
	<th style="color: #183292" colspan="3">Annual Term Summary</th>
	<th></th>
	<th></th>
	<th></th>
	</tr>
	...
	<td style="width: 30px"><img src="<?php echo base_url()."/uploads/result_labels/bf3.gif"; ?>" /></td>
	<td style="width: 30px"><img src="<?php echo base_url()."/uploads/result_labels/cum3.gif"; ?>" /></td>
	...
	<?php 
	    ...
	    foreach($first_result as $yin)
	    {
	        $firstTerm_subjects[$yin->subject]=array("totalscore"=>$yin->totalscore, "subject"=>$yin->subject);
	    }
	    
	    foreach($second_result as $yin2)
	    {
	        $secondTerm_subjects[$yin2->subject]=array("totalscore"=>$yin2->totalscore, "subject"=>$yin2->subject);
	    }
	    
	    //echo "<pre>"; print_r($firstTerm_subjects); echo "</pre>";
	?>
	<?php
	$totalCummScore = $total_divisors=0;
	$j=0; 
	foreach($result as $r) { ?>
	<tr>
	...
	<?php 
	   ...
	   if(!empty($firstTerm_subjects[$r->subject]))
	   {
	       $dvs1=1; 
	       $firstTerm=$firstTerm_subjects[$r->subject]['totalscore'];
	   }
	       
	   if(!empty($secondTerm_subjects[$r->subject]))
	   {
	       $dvs2=1; 
	       $secondTerm=$secondTerm_subjects[$r->subject]['totalscore'];
	   }
	    
	   $divisor=($dvs1+$dvs2+1);
	   $total_divisors+=$divisor;
	?>
	<td><p style="font-size: 10px;"><?php echo (double)$firstTerm + (double)$secondTerm; ?></p></td>
	<td><p style="font-size: 10px;"><?php echo (double)$firstTerm + (double)$secondTerm + $r->totalscore; $totalCummScore+= (double)$firstTerm + (double)$secondTerm+$r->totalscore; ?></p></td>
	<td><p style="font-size: 10px;"><?php echo round(( (double)$firstTerm + (double)$secondTerm + $r->totalscore)/$divisor,2); ?></p></td>
	<td><p style="font-size: 10px;"><?php echo round($average_score[$j],2); ?></p></td>
	<td><p style="font-size: 11px;">
	<?php for($i=0;$i<count($grading);$i++) {
		if((int) round(( (double)$firstTerm + (double)$secondTerm +$r->totalscore)/$divisor,2)<=(int) $grading[$i]->higher && (int) round(( (double)$firstTerm + (double)$secondTerm +$r->totalscore)/$divisor,2)>=(int) $grading[$i]->lower) {
	...
	<?php for($i=0;$i<count($grading);$i++) {
		if((int) round(( (double)$firstTerm + (double)$secondTerm+$r->totalscore)/$divisor,2)<=(int) $grading[$i]->higher && (int) round(((double)$firstTerm + (double)$secondTerm+$r->totalscore)/$num_of_terms,2)>=(int) $grading[$i]->lower) {
	...
	<tbody>
	<tr>
	<td><p style="font-weight: bold; font-size: 11px; color: #183292;">HANDWRITING</p></td>
	...
	<div class="cell" style="margin-left: -40px; margin-right: 20px">
	...
	<p style="border: 2px solid #183292; padding: 5px; font-size: 11px"><?php echo round($totalCummScore/$total_divisors, 2); ?></p>
	</div>
	...
	<div class="row">
	<div class="cell colspan5">
	<p style="font-size: 11px; font-weight: bold; color: #183292">HEAD TEACHER'S COMMENT: </p>
	</div>
	<div class="cell colspan7">
	<p style="border: 1px solid black; padding: 5px; font-size: 11px"><?php if(isset($principal_comment[0]->comment)) {echo $principal_comment[0]->comment;} ?><br /><br /><?php echo strtoupper($principalname[0]->name); ?></p>
	</div>
	</div>
</div>
</div>
