...
<?php foreach($schinfo->result() as $row)
	...
	<td><p style="font-size: 10px;"><?php echo $first_result[$j]->totalscore + $second_result[$j]->totalscore; ?></p></td>
	<td><p style="font-size: 10px;"><?php echo $first_result[$j]->totalscore + $second_result[$j]->totalscore + $r->totalscore; $totalCummScore+= $first_result[$j]->totalscore+$second_result[$j]->totalscore+$r->totalscore; ?></p></td>
	<td><p style="font-size: 10px;"><?php echo round(($first_result[$j]->totalscore + $second_result[$j]->totalscore + $r->totalscore)/$num_of_terms,2); ?></p></td>
	<td><p style="font-size: 10px;"><?php echo round($average_score[$j],2); ?></p></td>
	<td><p style="font-size: 11px;">
	<?php for($i=0;$i<count($grading);$i++) {
		if((int) round(($first_result[$j]->totalscore+$second_result[$j]->totalscore+$r->totalscore)/$num_of_terms,2)<=(int) $grading[$i]->higher && (int) round(($first_result[$j]->totalscore+$second_result[$j]->totalscore+$r->totalscore)/$num_of_terms,2)>=(int) $grading[$i]->lower) {
	...
	<p style="border: 2px solid #183292; padding: 5px; font-size: 11px"><?php echo round($totalCummScore/$num_of_terms, 2); ?></p>
	</div>
