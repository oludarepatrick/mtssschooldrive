...
<?php foreach($schinfo->result() as $row)
 ...
 foreach($broadsheet as $row) {?>
      ...
      <td class='result-row'><p style='font-size: 12px'><?php echo strtoupper($row[0]->student_id); ?></p></td>
        ...
        for($j=0;$j<count($subjects);$j++)
        {
          for($i=0;$i<count($row);$i++) {
            ...
            {
              $GLOBALS['check'] = 1;
              echo "<td class='result-row'><p style='font-size: 12px'>".$row[$i]->ca."</p></td>";
              break;
            }
            else
            {
        ...
        <?php if($average<10) {echo "<td class='result-row' style='color: red; font-face:bold;'><b>FAIL</b></td>";} else {echo "<td class='result-row' style='color: green; font-face:bold;'><b>PASS</b></td>";} ?>
        </tr>
        <?php } ?>
  ...
  <p style="color: blue"><a href="cbt_broadsheet">&lt;Back</a></p>
  </div>
  </div>
  </div>
  </div>
