...
function getBaseURL() {
	...
	base_url = window.location.protocol + "//" + window.location.host + "/" + pathArray[1] +"/";
    return base_url;
}	
...
function chatHeartbeat(){
	...
	jQuery.ajax({
	  url: base_url + "application/views/chat/chat.php?action=chatheartbeat",
	  cache: false,
	  dataType: "json",
...
function closeChatBox(chatboxtitle) {
	...
	jQuery.post(base_url + "application/views/chat/chat.php?action=closechat", { chatbox: chatboxtitle} , function(data){	
	});
...
function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle) {
	...
	if(event.keyCode == 13 && event.shiftKey == 0)  {
		...
		if (message != '') {
			jQuery.post( "http://localhost/eschoolin/application/views/chat/chat.php?action=sendchat", {to: chatboxtitle, message: message} , function(data){
...
function startChatSession(){  
	jQuery.ajax({
	  url: base_url + "application/views/chat/chat.php?action=startchatsession",
	  cache: false,
	  dataType: "json",
	  success: function(data) {
