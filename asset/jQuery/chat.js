...
function getBaseURL() {
	...
	for(var i =1; i <= pathArray.length - 1; i++ ){
		...
		if( pathArray[i] == 'index.php' || pathArray[i] == 'INDEX.PHP'){
			break;
		}else{
			appPath += pathArray[i] + "/";
		}
	...
	appPath = appPath+'chat/'
	base_url = window.location.protocol + "//" + window.location.host+appPath;
	//alert(base_url)
    return base_url;
}	
...
function restructureChatBoxes() {
	...
	for (x in chatBoxes) {
		...
		if (jQuery("#chatbox_"+chatboxtitle).css('display') != 'none') {
			if (align == 0) {
				jQuery("#chatbox_"+chatboxtitle).css('right', '250px'); //20
			} else {
				width = (align)*(225+7)+250;//20
...
function createChatBox(chatboxtitle,minimizeChatBox) {
	...
	if (chatBoxeslength == 0) {
		...
		width = (chatBoxeslength)*(225+7)+250;//20
		jQuery("#chatbox_"+chatboxtitle).css('right', width+'px');
	}
...
function closeChatBox(chatboxtitle) {
	...
	jQuery.post(base_url + "chat.php?action=closechat", { chatbox: chatboxtitle} , function(data){	
	});
...
function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle) {
	...
	if(event.keyCode == 13 && event.shiftKey == 0)  {
		...
		if (message != '') {
			jQuery.post(base_url + "chat.php?action=sendchat", {to: chatboxtitle, message: message} , function(data){
