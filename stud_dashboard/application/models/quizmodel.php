...
class QuizModel
{
    ...
    public function isExamAssigned($studId,$active_term,$active_session)
    {
        ...
        $sql = "SELECT a.`id`, a.`quiz_id`,b.`description`,b.`minutes`,b.`class_id`,b.`arm`,b.`subject_id`, a.`status`,b.`name`,b.status AS qstatus FROM `quiz_user` AS a,`quizzes` AS b WHERE a.`quiz_id`=b.`id` AND a.`user_id`=? AND b.`sessions`=? AND b.`terms`=? ORDER BY a.`id` DESC";
        $query = $this->db->prepare($sql);
        $query->execute(array($studId,$active_session,$active_term));
